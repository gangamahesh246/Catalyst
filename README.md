...
- `frontend/vercel.json` — Vercel config that proxies `/api/*` from the deployed frontend to the Render backend (no CORS, no code changes)

### 1. Backend → Render

1. Push the repo to GitHub (already done).
2. On [Render](https://render.com) → **New → Blueprint** → pick this repo. Render reads `render.yaml` and creates the `talentscout-backend` web service.
3. In the service's **Environment** tab, add your secret:
   - `GEMINI_API_KEY = <your key>` (optional — the agent falls back to the heuristic explainer if omitted)
4. Deploy. First build pulls scikit-learn etc. (~2 min). The service serves at `https://talentscout-backend-XXXX.onrender.com`.
5. Sanity-check: `https://<your-backend>.onrender.com/api/health` should return `{"status":"ok", ...}`.

> **Free-plan note:** Render's free web service spins down after 15 min idle. First request after a sleep takes ~30-60 s to wake. Upgrade to Starter ($7/mo) to keep it warm.

### 2. Frontend → Vercel

1. Edit `frontend/vercel.json` and replace `REPLACE-WITH-RENDER-URL` with your actual Render hostname:
   ```json
   "destination": "https://talentscout-backend-XXXX.onrender.com/api/:path*"
   ```
2. On [Vercel](https://vercel.com) → **Add New → Project** → import this repo.
3. In the project settings:
   - **Root Directory:** `frontend`
   - **Framework Preset:** Vite (auto-detected)
   - Build & output paths are already correct (`npm run build` → `dist`)
4. Deploy. Vercel reads `frontend/vercel.json` and rewrites every `/api/*` request to your Render backend.
5. Visit the Vercel URL — the React app calls `/api/scout` exactly like in dev, but Vercel transparently proxies to Render.

That's it. Locally you still get the Vite proxy → `localhost:8000`; in production you get the Vercel rewrite → Render. Same code path.
